...
/** Range Support */
// https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectGET.html#ExampleGetRangeRequestHeaders
function parseRangeString(rangeString) {
  // example: bytes=0-9
  var rangeValue = rangeString.split('=')[1];
  var pieces = rangeValue.split('-');

  return {
    start: parseInt(pieces[0]),
    end: parseInt(pieces[1]),
    size: parseInt(pieces[1]) - parseInt(pieces[0]) + 1
  };
}

function formatContentRange(rangeStart, bytesRead, fileSize) {
  let rangeEnd = rangeStart + (bytesRead - 1);
      ...
      return {
        promise: function () {
          return new Promise((resolve, reject) => {
            var args = [search, options, (err, result) => {
              if (err) {
                reject(err);
              } else {
                resolve(result);
            ...
            var cleanArgs = args.filter(item => !!item);
            wrapped.bind(self)(...cleanArgs);
          });
        },
        send: function (cb) {
...
S3Mock.prototype = {
  ...
  getObject: function (search, callback) {
    ...
    if (!callback) {
      ...
      if (search.Range) {
        var range = parseRangeString(search.Range);

        fs.open(path, 'r', function (err, fd) {
          if (!err) {
            var buffer = Buffer.alloc(range.size);
            fs.read(fd, buffer, 0, buffer.length, range.start, function (_err, bytesRead) {
              var stat = fs.statSync(path);
              ...
              var props = {
                ...
                ContentRange: formatContentRange(range.start, bytesRead, stat.size)
              };
