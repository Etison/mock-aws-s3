...
describe('S3', function () {
  ...
  it('should list files starting a marker with a partial filename', function (done) {
    ...
    s3.listObjects({
      Prefix: '', Bucket: __dirname + '/local/otters', Marker: 'mix/yay copy 10',
      Delimiter: '/'
    }, function (err, data) {
  ...
  it('should list files starting a marker with a partial filename (listV2)', function (done) {
    ...
    s3.listObjectsV2({
      Prefix: '', Bucket: __dirname + '/local/otters', StartAfter: 'mix/yay copy 10',
      Delimiter: '/'
    }, function (err, data) {
  ...
  context('can get portions of a file by range', function () {
    ...
    it("reads the beginning of file", (done) => {
      var req = s3.getObject({ Key: 'animal.txt', Bucket: __dirname + '/local/otters', Range: 'bytes=0-5' });
  ...
  it('should fail to put object tags against a non-existing object', function (done) {
    s3.putObjectTagging({
      Key: 'does-not-exist', Bucket: __dirname + '/local/otters',
      Tagging: { TagSet: [] }
    }, function (err, data) {
      expect(err).to.not.be.null;
      expect(err.statusCode).to.equal(404);
  ...
  it('should put object tags against an existing object', function (done) {
    s3.putObjectTagging({
      Key: 'animal.txt', Bucket: __dirname + '/local/otters',
